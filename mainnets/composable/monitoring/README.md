...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/composable/monitoring/composable-monitoring-install.sh)
```
...
# make the telegraf user sudo and adm to be able to execute scripts as composable user
...
```
cd $HOME
mkdir composable-mainnet-monitoring
cd composable-mainnet-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/composable/monitoring/monitor.sh
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/composable/monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/composable/monitoring/variables.sh
```
...
* full path to composable binary to COS_BIN_NAME ( check ```which centaurid```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_composable_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```centaurivaloper********```)
...
Create telegraf service for composable-mainnet-node monitoring
```
cat > /etc/systemd/system/telegraf_composable_mainnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_composable_mainnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_composable_mainnet.conf /etc/telegraf/telegraf_composable_mainnet.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_composable_mainnet
sudo systemctl restart telegraf_composable_mainnet
```
...
## [Dashboard link](https://monitoring-dashboards.l0vd.com/d/Composable_mainnet/composable-mainnet-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-1h&to=now)
