...
```
cd $HOME
mkdir composable-monitoring
cd composable-monitoring
...
* full path to composable binary to COS_BIN_NAME ( check ```which banksyd```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_composable_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```banksyvaloper********```)
...
Create telegraf service for composable-node monitoring
```
cat > /etc/systemd/system/telegraf_composable.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_composable.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf.conf /etc/telegraf/telegraf_composable.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_composable
sudo systemctl restart telegraf_composable
```
...
## [Dashboard link](http://95.216.2.219:3000/d/Composable/composable-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-5m&to=now)
