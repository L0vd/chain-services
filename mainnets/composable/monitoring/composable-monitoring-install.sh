...
echo ''
echo -e 'INSTALLING ${ALL_CAP_NAME} NODE MONITORING'
...
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/banksyd or /home/user/go/bin/banksyd
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: banksyvaloper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
cat > telegraf_composable.conf <<EOL
...
[[outputs.influxdb]]
  ...
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/composable-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
sudo mv telegraf_composable.conf /etc/telegraf/telegraf_composable.conf
...
echo ''
if [[ `sudo systemctl status telegraf_composable | grep active` =~ "running" ]]; then
...
echo ''
echo -e '\e[7mYour ${FIRST_CAP_NAME} node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_composable -f\e[0m'