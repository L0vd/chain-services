...
#gitopia monitoring variables template 
...
[[outputs.influxdb]]
  database = "gitopiametricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/gitopia-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
sudo mv telegraf_gitopia.conf /etc/telegraf/telegraf_gitopia.conf
...
echo ''
if [[ `sudo systemctl status telegraf_gitopia | grep active` =~ "running" ]]; then
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_gitopia -f\e[0m'
