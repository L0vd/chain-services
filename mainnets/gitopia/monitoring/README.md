...
```
cd $HOME
mkdir gitopia-monitoring
cd gitopia-monitoring
...
Create telegraf service for gitopia-node monitoring
```
cat > /etc/systemd/system/telegraf_gitopia.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_gitopia.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf.conf /etc/telegraf/telegraf_gitopia.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_gitopia
sudo systemctl restart telegraf_gitopia
```
...
## [Dashboard link](http://95.216.2.219:3000/d/Gitopia/gitopia-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-5m&to=now)
