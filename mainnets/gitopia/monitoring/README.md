...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/gitopia/monitoring/gitopia-monitoring-install.sh)
```
...
# make the telegraf user sudo and adm to be able to execute scripts as gitopia user
...
```
cd $HOME
mkdir gitopia-mainnet-monitoring
cd gitopia-mainnet-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/gitopia/monitoring/monitor.sh
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/gitopia/monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/gitopia/monitoring/variables.sh
```
...
* full path to gitopia binary to COS_BIN_NAME ( check ```which gitopiad```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_gitopia_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```gitopiavaloper********```)
...
Create telegraf service for gitopia-mainnet-node monitoring
```
cat > /etc/systemd/system/telegraf_gitopia_mainnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_gitopia_mainnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_gitopia_mainnet.conf /etc/telegraf/telegraf_gitopia_mainnet.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_gitopia_mainnet
sudo systemctl restart telegraf_gitopia_mainnet
```
...
## [Dashboard link](https://monitoring-dashboards.l0vd.com/d/Gitopia_mainnet/gitopia-mainnet-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-1h&to=now)
