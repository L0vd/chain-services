...
cd $HOME
mkdir bitcanna-monitoring
cd bitcanna-monitoring
...
#Bitcanna monitoring variables template 
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/bcnad or /home/user/go/bin/bcnad
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: bcnavaloper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
cat > telegraf_bitcanna.conf <<EOL
...
[[outputs.influxdb]]
  database = "bitcannametricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/bitcanna-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
cat > /etc/systemd/system/telegraf_bitcanna.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_bitcanna.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_bitcanna.conf /etc/telegraf/telegraf_bitcanna.conf


sudo systemctl daemon-reload
sudo systemctl enable telegraf_bitcanna
sudo systemctl restart telegraf_bitcanna
sleep 4
...
echo ''
if [[ `sudo systemctl status telegraf_bitcanna | grep active` =~ "running" ]]; then
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_bitcanna -f\e[0m'
