...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/haqq/monitoring/haqq-monitoring-install.sh)
```
...
```
cd $HOME
mkdir haqq-mainnet-monitoring
cd haqq-mainnet-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/haqq/monitoring/monitor.sh
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/haqq/monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/haqq/monitoring/variables.sh
```
...
* full path to haqq binary to COS_BIN_NAME ( check ```which d```)
...
Create telegraf service for haqq-mainnet-node monitoring
```
cat > /etc/systemd/system/telegraf_haqq_mainnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_haqq_mainnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_haqq_mainnet.conf /etc/telegraf/telegraf_haqq_mainnet.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_haqq_mainnet
sudo systemctl restart telegraf_haqq_mainnet
```
...
## [Dashboard link](https://monitoring-dashboards.l0vd.com/d/Haqq_mainnet/haqq-mainnet-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-1h&to=now)
