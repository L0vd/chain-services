...
Create telegraf service for haqq-node monitoring
```
cat > /etc/systemd/system/telegraf_haqq.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_haqq.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf.conf /etc/telegraf/telegraf_haqq.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_haqq
sudo systemctl restart telegraf_haqq
```
...
## [Dashboard link](http://95.216.2.219:3000/d/Haqq/haqq-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-5m&to=now)
