...
#HAQQ monitoring variables template 
...
sudo mv telegraf_haqq.conf /etc/telegraf/telegraf_haqq.conf
...
echo ''
if [[ `sudo systemctl status telegraf_haqq | grep active` =~ "running" ]]; then
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_haqq -f\e[0m'
