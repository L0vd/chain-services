...
cat > telegraf_haqq_mainnet.conf <<EOL
...
[[outputs.influxdb]]
  database = "haqq_mainnet_metricsdb"
  urls = [ "http://monitoring-db.l0vd.com:80" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/haqq-mainnet-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
echo ''
if [[ `sudo systemctl status telegraf_haqq_mainnet | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf_haqq_mainnet -f\e[0m"
fi
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_haqq_mainnet -f\e[0m'