...
echo ''
echo -e 'INSTALLING UMEE NODE MONITORING'
...
cd $HOME
mkdir umee-mainnet-monitoring
cd umee-mainnet-monitoring
...
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/umeed or /home/user/go/bin/umeed
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: umeevaloper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
cat > telegraf_umee_mainnet.conf <<EOL
...
[[outputs.influxdb]]
  database = "umee_mainnet_metricsdb"
  urls = [ "http://monitoring-db.l0vd.com:80" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/umee-mainnet-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
cat > /etc/systemd/system/telegraf_umee_mainnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_umee_mainnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_umee_mainnet.conf /etc/telegraf/telegraf_umee_mainnet.conf


sudo systemctl daemon-reload
sudo systemctl enable telegraf_umee_mainnet
sudo systemctl restart telegraf_umee_mainnet
sleep 4
...
echo ''
if [[ `sudo systemctl status telegraf_umee_mainnet | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf_umee_mainnet -f\e[0m"
fi

echo ''
echo -e '\e[7mYour UMEE node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_umee_mainnet -f\e[0m'