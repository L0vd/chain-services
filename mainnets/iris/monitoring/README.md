...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/iris/monitoring/iris-monitoring-install.sh)
```
...
# make the telegraf user sudo and adm to be able to execute scripts as iris user
...
```
cd $HOME
mkdir iris-mainnet-monitoring
cd iris-mainnet-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/iris/monitoring/monitor.sh
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/iris/monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/iris/monitoring/variables.sh
```
...
* full path to iris binary to COS_BIN_NAME ( check ```which irisd```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_iris_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```iaavaloper********```)
...
Create telegraf service for iris-mainnet-node monitoring
```
cat > /etc/systemd/system/telegraf_iris_mainnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_iris_mainnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_iris_mainnet.conf /etc/telegraf/telegraf_iris_mainnet.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_iris_mainnet
sudo systemctl restart telegraf_iris_mainnet
```
...
## [Dashboard link](https://monitoring-dashboards.l0vd.com/d/Iris_mainnet/iris-mainnet-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-1h&to=now)
