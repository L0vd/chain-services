...
echo ''
echo -e 'INSTALLING IRIS NODE MONITORING'
...
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/irisd or /home/user/go/bin/irisd
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: iaavaloper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
cat > telegraf_iris_mainnet.conf <<EOL
...
[[outputs.influxdb]]
  database = "iris_mainnet_metricsdb"
  urls = [ "http://monitoring-db.l0vd.com:80" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/iris-mainnet-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
echo ''
if [[ `sudo systemctl status telegraf_iris_mainnet | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf_iris_mainnet -f\e[0m"
fi

echo ''
echo -e '\e[7mYour IRIS node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_iris_mainnet -f\e[0m'