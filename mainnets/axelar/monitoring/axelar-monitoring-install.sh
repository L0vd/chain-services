...
echo ''
echo -e 'INSTALLING AXELAR NODE MONITORING'
...
cd $HOME
mkdir axelar-mainnet-monitoring
cd axelar-mainnet-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/mainnets/axelar/monitoring/monitor.sh


cat > variables.sh <<EOL
#${FIRST_CAP_NAME} monitoring variables template 
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/axelard or /home/user/go/bin/axelard
...
cat > telegraf_axelar_mainnet.conf <<EOL
...
[[outputs.influxdb]]
  database = "axelar_mainnet_metricsdb"
  urls = [ "http://monitoring-db.l0vd.com:80" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/axelar-mainnet-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
cat > /etc/systemd/system/telegraf_axelar_mainnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_axelar_mainnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_axelar_mainnet.conf /etc/telegraf/telegraf_axelar_mainnet.conf


sudo systemctl daemon-reload
sudo systemctl enable telegraf_axelar_mainnet
sudo systemctl restart telegraf_axelar_mainnet
sleep 4
...
echo ''
if [[ `sudo systemctl status telegraf_axelar_mainnet | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf_axelar_mainnet -f\e[0m"
fi

echo ''
echo -e '\e[7mYour AXELAR node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_axelar_mainnet -f\e[0m'