...
echo ''
echo -e 'INSTALLING  NODE MONITORING'
...
# make the telegraf user sudo and adm to be able to execute scripts as  user
...
mkdir --monitoring
cd --monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/s//monitoring/monitor.sh


cat > variables.sh <<EOL
#${FIRST_CAP_NAME} monitoring variables template 
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/ or /home/user/go/bin/
...
[[outputs.influxdb]]
  database = "__metricsdb"
  urls = [ "http://monitoring-db.l0vd.com:80" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/--monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
cat > /etc/systemd/system/telegraf__.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf__.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf__.conf /etc/telegraf/telegraf__.conf


sudo systemctl daemon-reload
sudo systemctl enable telegraf__
sudo systemctl restart telegraf__
sleep 4
...
echo ''
if [[ `sudo systemctl status telegraf__ | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf__ -f\e[0m"
fi

echo ''
echo -e '\e[7mYour  node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf__ -f\e[0m'