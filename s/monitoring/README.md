...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/s//monitoring/-monitoring-install.sh)
```
...
wget https://raw.githubusercontent.com/L0vd/chain-services/main/s//monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/s//monitoring/variables.sh
```
...
* full path to  binary to COS_BIN_NAME ( check ```which ```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to__node_config/config/config.toml```)
...
Create telegraf service for --node monitoring
```
...
## [Dashboard link](https://monitoring-dashboards.l0vd.com/d/_/--monitoring-by-l0vd?orgId=1&refresh=30s&from=now-1h&to=now)
