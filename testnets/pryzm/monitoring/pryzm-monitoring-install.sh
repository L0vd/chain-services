...
echo ''
echo -e 'INSTALLING PRYZM NODE MONITORING'
...
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/pryzmd or /home/user/go/bin/pryzmd
...
cat > telegraf_pryzm_testnet.conf <<EOL
...
[[outputs.influxdb]]
  database = "pryzm_testnet_metricsdb"
  urls = [ "http://monitoring-db.l0vd.com:80" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/pryzm-testnet-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
echo ''
if [[ `sudo systemctl status telegraf_pryzm_testnet | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf_pryzm_testnet -f\e[0m"
fi

echo ''
echo -e '\e[7mYour PRYZM node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_pryzm_testnet -f\e[0m'