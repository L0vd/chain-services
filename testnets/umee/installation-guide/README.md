...
### Configure your node
```
d config chain-id ${UMEE_CHAIN_ID}
```

### Initialize your node
```
d init ${UMEE_NODENAME} --chain-id ${UMEE_CHAIN_ID}
```
...
d tendermint unsafe-reset-all --home $HOME/.umee --keep-addr-book
...
#### 1. Add a new key
```
d keys add ${UMEE_WALLET}
```
##### (OR)

#### 1. Recover your key
```
d keys add ${UMEE_WALLET} --recover
```

```
UMEE_WALLET_ADDR=$(d keys show ${UMEE_WALLET} -a)
