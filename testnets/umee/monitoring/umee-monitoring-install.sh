...
[[outputs.influxdb]]
  ...
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/umee-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
cat > /etc/systemd/system/telegraf_umee.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_umee.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_umee.conf /etc/telegraf/telegraf_umee.conf


sudo systemctl daemon-reload
sudo systemctl enable telegraf_umee
sudo systemctl restart telegraf_umee
sleep 4
...
echo ''
if [[ `sudo systemctl status telegraf_umee | grep active` =~ "running" ]]; then
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_umee -f\e[0m'
