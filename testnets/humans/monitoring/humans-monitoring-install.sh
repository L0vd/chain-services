...
echo ''
echo -e 'INSTALLING HUMANS NODE MONITORING'
...
# make the telegraf user sudo and adm to be able to execute scripts as humans user
...
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/humans/monitoring/monitor.sh >/dev/null 2>&1
...
#Humans monitoring variables template 
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/humansd or /home/user/go/bin/humansd
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: human@valoper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
[[outputs.influxdb]]
  database = "humansmetricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/humans-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
cat > /etc/systemd/system/telegraf_humans.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_humans.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_humans.conf /etc/telegraf/telegraf_humans.conf


sudo systemctl daemon-reload
sudo systemctl enable telegraf_humans
sudo systemctl restart telegraf_humans
sleep 4
...
echo ''
if [[ `sudo systemctl status telegraf_humans | grep active` =~ "running" ]]; then
...
echo ''
echo -e '\e[7mYour Humans node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_humans -f\e[0m'
