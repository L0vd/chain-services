...
* full path to humans binary to COS_BIN_NAME ( check ```which humansd```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_humans_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```humanvaloper********```)
...
Create telegraf service for humans-node monitoring
```
...
sudo mv telegraf.conf /etc/telegraf/telegraf_humans.conf
```
...
## [Dashboard link](http://95.216.2.219:3000/d/Humans/humans-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-5m&to=now)
