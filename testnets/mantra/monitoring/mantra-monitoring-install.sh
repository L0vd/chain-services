...
echo ''
echo -e 'INSTALLING MANTRA NODE MONITORING'
...
# make the telegraf user sudo and adm to be able to execute scripts as mantra user
...
cd $HOME
mkdir mantra-testnet-monitoring
cd mantra-testnet-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/mantra/monitoring/monitor.sh


cat > variables.sh <<EOL
#${FIRST_CAP_NAME} monitoring variables template 
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/mantrachaind or /home/user/go/bin/mantrachaind
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: mantravaloper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
cat > telegraf_mantra_testnet.conf <<EOL
...
[[outputs.influxdb]]
  database = "mantra_testnet_metricsdb"
  urls = [ "http://monitoring-db.l0vd.com:80" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/mantra-testnet-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
cat > /etc/systemd/system/telegraf_mantra_testnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_mantra_testnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_mantra_testnet.conf /etc/telegraf/telegraf_mantra_testnet.conf


sudo systemctl daemon-reload
sudo systemctl enable telegraf_mantra_testnet
sudo systemctl restart telegraf_mantra_testnet
sleep 4
...
echo ''
if [[ `sudo systemctl status telegraf_mantra_testnet | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf_mantra_testnet -f\e[0m"
fi

echo ''
echo -e '\e[7mYour MANTRA node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_mantra_testnet -f\e[0m'