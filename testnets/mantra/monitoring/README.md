...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/mantra/monitoring/mantra-monitoring-install.sh)
```
...
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/mantra/monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/mantra/monitoring/variables.sh
```
...
* full path to mantra binary to COS_BIN_NAME ( check ```which mantrachaind```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_mantra_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```mantravaloper********```)
...
Create telegraf service for mantra-testnet-node monitoring
```
...
## [Dashboard link](https://monitoring-dashboards.l0vd.com/d/Mantra_testnet/mantra-testnet-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-1h&to=now)
...
### Version
Version of mantrachaind binary
