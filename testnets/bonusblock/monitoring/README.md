...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/bonusblock/monitoring/bonusblock-monitoring-install.sh)
```
...
# make the telegraf user sudo and adm to be able to execute scripts as bonusblock user
...
```
cd $HOME
mkdir bonusblock-monitoring
cd bonusblock-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/bonusblock/monitoring/monitor.sh
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/bonusblock/monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/bonusblock/monitoring/variables.sh
```
...
* full path to bonusblock binary to COS_BIN_NAME ( check ```which bonus-blockd```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_bonusblock_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```bonusvaloper********```)
...
Create telegraf service for bonusblock-node monitoring
```
cat > /etc/systemd/system/telegraf_bonusblock.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_bonusblock.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf.conf /etc/telegraf/telegraf_bonusblock.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_bonusblock
sudo systemctl restart telegraf_bonusblock
```
...
## [Dashboard link](http://95.216.2.219:3000/d/Bonusblock/bonusblock-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-5m&to=now)
...
### Version
Version of bonus-blockd binary
