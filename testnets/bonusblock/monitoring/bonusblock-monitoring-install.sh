...
echo ''
echo -e 'INSTALLING BONUSBLOCK NODE MONITORING'
...
#Bonusblock monitoring variables template 
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/bonus-blockd or /home/user/go/bin/bonus-blockd
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: bonusvaloper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
cat > telegraf_bonusblock.conf <<EOL
...
[[outputs.influxdb]]
  database = "bonusblockmetricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/bonusblock-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
sudo mv telegraf_bonusblock.conf /etc/telegraf/telegraf_bonusblock.conf
...
echo ''
if [[ `sudo systemctl status telegraf_bonusblock | grep active` =~ "running" ]]; then
...
echo ''
echo -e '\e[7mYour Bonusblock node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_bonusblock -f\e[0m'
