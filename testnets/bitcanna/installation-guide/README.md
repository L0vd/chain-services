...
### Configure your node
```
nulld config chain-id ${BITCANNA_CHAIN_ID}
```

### Initialize your node
```
nulld init ${BITCANNA_NODENAME} --chain-id ${BITCANNA_CHAIN_ID}
```
...
```
```
nulld config node tcp://localhost:${BITCANNA_PORT}657
```
...
nulld tendermint unsafe-reset-all --home $HOME/.bcna --keep-addr-book
...
#### 1. Add a new key
```
nulld keys add ${BITCANNA_WALLET}
```
##### (OR)

#### 1. Recover your key
```
nulld keys add ${BITCANNA_WALLET} --recover
```

```
BITCANNA_WALLET_ADDR=$(nulld keys show ${BITCANNA_WALLET} -a)
