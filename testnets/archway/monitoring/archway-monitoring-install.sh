...
echo ''
echo -e 'INSTALLING ARCHWAY NODE MONITORING'
...
cd $HOME
mkdir archway-testnet-monitoring
cd archway-testnet-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/archway/monitoring/monitor.sh
...
cat > telegraf_archway_testnet.conf <<EOL
...
[[outputs.influxdb]]
  database = "archway_testnet_metricsdb"
  urls = [ "http://monitoring-db.l0vd.com:80" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/archway-testnet-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
cat > /etc/systemd/system/telegraf_archway_testnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_archway_testnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_archway_testnet.conf /etc/telegraf/telegraf_archway_testnet.conf


sudo systemctl daemon-reload
sudo systemctl enable telegraf_archway_testnet
sudo systemctl restart telegraf_archway_testnet
sleep 4
...
echo ''
if [[ `sudo systemctl status telegraf_archway_testnet | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf_archway_testnet -f\e[0m"
fi

echo ''
echo -e '\e[7mYour ARCHWAY node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_archway_testnet -f\e[0m'