...
sudo mv telegraf_archway.conf /etc/telegraf/telegraf_archway.conf
...
echo ''
if [[ `sudo systemctl status telegraf_archway | grep active` =~ "running" ]]; then
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_archway -f\e[0m'
