...
Create telegraf service for archway-node monitoring
```
cat > /etc/systemd/system/telegraf_archway.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_archway.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf.conf /etc/telegraf/telegraf_archway.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_archway
sudo systemctl restart telegraf_archway
```
...
## [Dashboard link](http://95.216.2.219:3000/d/Archway/archway-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-5m&to=now)
