...
echo ''
echo -e 'INSTALLING NORIA NODE MONITORING'
...
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/noriad or /home/user/go/bin/noriad
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: noriavaloper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
cat > telegraf_noria_testnet.conf <<EOL
...
[[outputs.influxdb]]
  database = "noria_testnet_metricsdb"
  urls = [ "http://monitoring-db.l0vd.com:80" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/noria-testnet-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
echo ''
if [[ `sudo systemctl status telegraf_noria_testnet | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf_noria_testnet -f\e[0m"
fi

echo ''
echo -e '\e[7mYour NORIA node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_noria_testnet -f\e[0m'