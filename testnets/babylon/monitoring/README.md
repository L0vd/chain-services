...
* full path to babylon binary to COS_BIN_NAME ( check ```which babylond```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_babylon_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```bbnvaloper********```)
...
Create telegraf service for babylon-testnet-node monitoring
```
...
## [Dashboard link](https://monitoring-dashboards.l0vd.com/d/Babylon_testnet/babylon-testnet-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-1h&to=now)
