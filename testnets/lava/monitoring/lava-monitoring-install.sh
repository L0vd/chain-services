...
#Lava monitoring variables template 
...
[[outputs.influxdb]]
  database = "lavametricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/lava-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
sudo mv telegraf_lava.conf /etc/telegraf/telegraf_lava.conf
...
echo ''
if [[ `sudo systemctl status telegraf_lava | grep active` =~ "running" ]]; then
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_lava -f\e[0m'
