...
echo ''
echo -e 'INSTALLING LAVA NODE MONITORING'
...
cd $HOME
mkdir lava-testnet-monitoring
cd lava-testnet-monitoring
...
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/lavad or /home/user/go/bin/lavad
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: lava@valoper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
cat > telegraf_lava_testnet.conf <<EOL
...
[[outputs.influxdb]]
  database = "lava_testnet_metricsdb"
  urls = [ "http://monitoring-db.l0vd.com:80" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/lava-testnet-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
cat > /etc/systemd/system/telegraf_lava_testnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_lava_testnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_lava_testnet.conf /etc/telegraf/telegraf_lava_testnet.conf


sudo systemctl daemon-reload
sudo systemctl enable telegraf_lava_testnet
sudo systemctl restart telegraf_lava_testnet
sleep 4
...
echo ''
if [[ `sudo systemctl status telegraf_lava_testnet | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf_lava_testnet -f\e[0m"
fi

echo ''
echo -e '\e[7mYour LAVA node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_lava_testnet -f\e[0m'