...
echo ''
echo -e 'INSTALLING artela NODE MONITORING'
...
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/artelad or /home/user/go/bin/artelad
...
cat > telegraf_artela_testnet.conf <<EOL
...
echo ''
if [[ `sudo systemctl status telegraf_artela_testnet | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf_artela_testnet -f\e[0m"
fi

echo ''
echo -e '\e[7mYour artela node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_artela_testnet -f\e[0m'