...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/artela/monitoring/artela-monitoring-install.sh)
```
...
# make the telegraf user sudo and adm to be able to execute scripts as artela user
...
```
cd $HOME
mkdir artela-testnet-monitoring
cd artela-testnet-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/artela/monitoring/monitor.sh
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/artela/monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/artela/monitoring/variables.sh
```
...
* full path to artela binary to COS_BIN_NAME ( check ```which artelad```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_artela_node_config/config/config.toml```)
...
Create telegraf service for artela-testnet-node monitoring
```
cat > /etc/systemd/system/telegraf_artela_testnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_artela_testnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_artela_testnet.conf /etc/telegraf/telegraf_artela_testnet.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_artela_testnet
sudo systemctl restart telegraf_artela_testnet
```
...
## [Dashboard link](https://monitoring-dashboards.l0vd.com/d/Artela_testnet/artela-testnet-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-1h&to=now)
