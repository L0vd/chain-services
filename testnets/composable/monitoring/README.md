...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/composable/monitoring/composable-monitoring-install.sh)
```
...
```
cd $HOME
mkdir composable-testnet-monitoring
cd composable-testnet-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/composable/monitoring/monitor.sh
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/composable/monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/composable/monitoring/variables.sh
```
...
Create telegraf service for composable-testnet-node monitoring
```
cat > /etc/systemd/system/telegraf_composable_testnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_composable_testnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_composable_testnet.conf /etc/telegraf/telegraf_composable_testnet.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_composable_testnet
sudo systemctl restart telegraf_composable_testnet
```
...
## [Dashboard link](https://monitoring-dashboards.l0vd.com/d/Composable_testnet/composable-testnet-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-1h&to=now)
