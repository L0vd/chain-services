...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/andromeda/monitoring/andromeda-monitoring-install.sh)
```
...
# make the telegraf user sudo and adm to be able to execute scripts as andromeda user
...
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/andromeda/monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/andromeda/monitoring/variables.sh
```
...
* full path to andromeda binary to COS_BIN_NAME ( check ```which andromedad```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_andromeda_node_config/config/config.toml```)
...
Create telegraf service for andromeda-node monitoring
```
cat > /etc/systemd/system/telegraf_andromeda.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_andromeda.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf.conf /etc/telegraf/telegraf_andromeda.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_andromeda
sudo systemctl restart telegraf_andromeda
```
...
## [Dashboard link](http://95.216.2.219:3000/d/Andromeda/andromeda-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-5m&to=now)
