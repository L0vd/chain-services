...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/selfchain/monitoring/selfchain-monitoring-install.sh)
```
...
# make the telegraf user sudo and adm to be able to execute scripts as selfchain user
...
```
cd $HOME
mkdir selfchain-testnet-monitoring
cd selfchain-testnet-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/selfchain/monitoring/monitor.sh
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/selfchain/monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/selfchain/monitoring/variables.sh
```
...
* full path to selfchain binary to COS_BIN_NAME ( check ```which d```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_selfchain_node_config/config/config.toml```)
...
Create telegraf service for selfchain-testnet-node monitoring
```
cat > /etc/systemd/system/telegraf_selfchain_testnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_selfchain_testnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_selfchain_testnet.conf /etc/telegraf/telegraf_selfchain_testnet.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_selfchain_testnet
sudo systemctl restart telegraf_selfchain_testnet
```
...
## [Dashboard link](https://monitoring-dashboards.l0vd.com/d/Selfchain_testnet/selfchain-testnet-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-1h&to=now)
