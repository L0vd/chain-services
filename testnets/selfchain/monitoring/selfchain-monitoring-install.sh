...
echo ''
echo -e 'INSTALLING SELFCHAIN NODE MONITORING'
...
cat > telegraf_selfchain_testnet.conf <<EOL
...
[[outputs.influxdb]]
  database = "selfchain_testnet_metricsdb"
  urls = [ "http://monitoring-db.l0vd.com:80" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/selfchain-testnet-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
echo ''
if [[ `sudo systemctl status telegraf_selfchain_testnet | grep active` =~ "running" ]]; then
  ...
  echo -e "You can check telegraf logs by following command \e[7msudo journalctl -u telegraf_selfchain_testnet -f\e[0m"
fi

echo ''
echo -e '\e[7mYour SELFCHAIN node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_selfchain_testnet -f\e[0m'