...
echo ''
echo -e 'INSTALLING Ojo NODE MONITORING'
...
#ojo monitoring variables template 
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/ojod or /home/user/go/bin/ojod
...
[[outputs.influxdb]]
  database = "ojometricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/ojo-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
sudo mv telegraf_ojo.conf /etc/telegraf/telegraf_ojo.conf
...
echo ''
if [[ `sudo systemctl status telegraf_ojo | grep active` =~ "running" ]]; then
...
echo ''
echo -e '\e[7mYour Ojo node monitoring is installed!\e[0m'
echo ''
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_ojo -f\e[0m'
