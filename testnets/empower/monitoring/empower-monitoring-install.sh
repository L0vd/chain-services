...
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/empowerd or /home/user/go/bin/empowerd
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: empowervaloper1234545636767376535673
LOGENTRY=$LOGENTRY
EOL
...
[[outputs.influxdb]]
  database = "empowermetricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/empower-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
sudo mv telegraf_empower.conf /etc/telegraf/telegraf_empower.conf
...
echo ''
if [[ `sudo systemctl status telegraf_empower | grep active` =~ "running" ]]; then
...
echo -e ''
echo -e 'Check telegraf logs: \e[7msudo journalctl -u telegraf_empower -f\e[0m'