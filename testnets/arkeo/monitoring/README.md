...
```
. <(wget -qO- https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/arkeo/monitoring/arkeo-monitoring-install.sh)
```
...
# make the telegraf user sudo and adm to be able to execute scripts as arkeo user
...
```
cd $HOME
mkdir arkeo-testnet-monitoring
cd arkeo-testnet-monitoring
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/arkeo/monitoring/monitor.sh
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/arkeo/monitoring/telegraf.conf
wget https://raw.githubusercontent.com/L0vd/chain-services/main/testnets/arkeo/monitoring/variables.sh
```
...
* full path to arkeo binary to COS_BIN_NAME ( check ```which arkeod```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_arkeo_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```tarkeovaloper********```)
...
Create telegraf service for arkeo-testnet-node monitoring
```
cat > /etc/systemd/system/telegraf_arkeo_testnet.service <<EOL
[Unit]
...
User=telegraf
ExecStart=/usr/bin/telegraf -config /etc/telegraf/telegraf_arkeo_testnet.conf -config-directory /etc/telegraf/telegraf.d $TELEGRAF_OPTS
...
sudo mv telegraf_arkeo_testnet.conf /etc/telegraf/telegraf_arkeo_testnet.conf
```
Restart telegraf service

```
sudo systemctl daemon-reload
sudo systemctl enable telegraf_arkeo_testnet
sudo systemctl restart telegraf_arkeo_testnet
```
...
## [Dashboard link](https://monitoring-dashboards.l0vd.com/d/Arkeo_testnet/arkeo-testnet-monitoring-by-l0vd?orgId=1&refresh=30s&from=now-1h&to=now)
